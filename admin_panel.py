...
def create_admin(username, password, role='admin'):
    ...
    try:
        c.execute('INSERT INTO admin_credentials VALUES (?,?,?,?,?)',
                  (admin_id, username, hash_password(password), role, datetime.now()))
        conn.commit()
        conn.close()
        return True
    except:
...
def auth_admin(username, password):
    ...
    conn = sqlite3.connect('umrah_pro.db')
    c = conn.cursor()
    c.execute('SELECT admin_id, role FROM admin_credentials WHERE username=? AND password_hash=?',
...
def get_dashboard_stats():
    ...
    stats['active_packages'] = c.fetchone()[0]
    ...
    stats['paid_bookings'] = c.fetchone()[0]
    ...
    stats['total_inquiries'] = c.fetchone()[0]
    
    c.execute("SELECT COUNT(*) FROM package_inquiries WHERE status='Pending'")
    stats['pending_inquiries'] = c.fetchone()[0]
...
if not st.session_state.admin_logged_in:
    ...
    with col2:
        ...
        with st.form("admin_login"):
            ...
            if submit:
                result = auth_admin(username, password)
                if result:
    ...
    st.markdown(f"""
    <div class="main-header">
        <h1>⚙️ Admin Panel - Welcome {st.session_state.admin_username}!</h1>
        <p>Manage Umrah Pro Platform</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    **EXCLUSIVE LAUNCH BONUS:**
    ...
    # ========== SETTINGS ==========
        ...
        with settings_tabs[0]:
            ...
            conn = sqlite3.connect('umrah_pro.db')
            c = conn.cursor()
            c.execute("SELECT admin_id, username, role, created_at FROM admin_credentials")
            admins = c.fetchall()
            conn.close()
            ...
            for admin in admins:
                ...
                with col1:
                    st.write(f"**{admin[1]}** - {admin[2]}")
                    st.caption(f"Created: {admin[3]}")
                
                with col2:
                    if admin[1] != st.session_state.admin_username:
                        if st.button(f"🗑️ Delete", key=f"del_admin_{admin[0]}"):
                            conn = sqlite3.connect('umrah_pro.db')
                            c = conn.cursor()
                            c.execute("DELETE FROM admin_credentials WHERE admin_id=?", (admin[0],))
                            conn.commit()
                            conn.close()
            ...
            with st.form("add_admin_form"):
                ...
                confirm_admin_password = st.text_input("Confirm Password", type="password")
                admin_role = st.selectbox("Role", ["admin", "super_admin"])
                
                submit_admin = st.form_submit_button("➕ Add Admin", type="primary")
                ...
                if submit_admin:
                    if len(new_admin_password) < 8:
                        ...
                        if create_admin(new_admin_username, new_admin_password, admin_role):
